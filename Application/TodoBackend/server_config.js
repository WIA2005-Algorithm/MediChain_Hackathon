...
app.post('/api/update-token', (req, res) => {
    ...
    Superuser.findOne({refreshToken: req.body.refreshToken}).exec((_, doc) => {
        if(!doc)
        return res.status(err.status).json(new response.errorResponse(err));
        jwt.verify(req.body.refreshToken, REFRESH_ACCESS_TOKEN, (_, user) => {
            if(!user) return res.status(err.status).json(new response.errorResponse(err));
            return res.status(200).json({accessToken:getAccessToken({username: user.username, role: user.role}, false), refreshToken: req.body.refreshToken})
        })
    });
})
