...
export const buildCAClient = (FabricCAServices, ccp, orgName) => { 
	...
	console.log(caInfo);
	if(!caInfo)
	throw errors.file_not_found.withDetails("Configuration Files are missing");
...
export const enrollAdmin = async (caClient, wallet, orgName, ccp) => {
	...
	const orgMspId = ccp.organizations[orgName].mspid;
		...
		if (identity)
		throw errors.duplicate_identity.withDetails('An identity for the admin user already exists in the wallet.');
		// Enroll the admin user, and import the new identity into the wallet.
		const enrollment = await caClient.enroll({ enrollmentID: user, enrollmentSecret: pass });
		const x509Identity = {
			credentials: {
...
export const registerAndEnrollUser = async (caClient, wallet, orgName, userId, affiliation, ccp) => {
		// Check to see if we've already enrolled the user
		const userIdentity = await wallet.get(userId);
		const admin = ccp.organizations[orgName].adminUserId;
		const orgMspId = ccp.organizations[orgName].mspid;
		if (userIdentity) 
			throw errors.duplicate_identity.withDetails('An identity for the requested user already exists in the wallet.');

		// Must use an admin to register a new user
		const adminIdentity = await wallet.get(admin);
		if (!adminIdentity)
		throw errors.required_admin.withDetails('The organisation admin is missing in the wallet. Enroll admin before registering users.');
