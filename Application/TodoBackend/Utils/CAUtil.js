...
export const buildCAClient = (FabricCAServices, ccp, orgName) => {
  ...
  const caInfo =
    ccp.certificateAuthorities[ccp.organizations[orgName].certificateAuthorities[0]]; //lookup CA details from config
  if (!caInfo) throw errors.file_not_found.withDetails("Configuration Files are missing");
