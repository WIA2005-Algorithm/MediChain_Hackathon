...
export function generateNetworkFiles(execStr, NetworkName) {
  ...
  const dir = shell
    .exec(`find ${ROOT_APP_DIR} -name \"${CREATE_NETWORK_TEMPLATE_FOLDER}\" 2>/dev/null`)
    .stdout.trim();
  ...
  shell.exec(`echo "${PASSWORD}" | sudo -S "./networkDown.sh"`, function (code) {
    if (code != "0")
      setNetworkStatus(NetworkName, {
        ...
        description: "Fabric Network has failed stopped successfully"
      }).finally(() =>
        log(
          "SuperAdmin",
      ...
      setNetworkStatus(NetworkName, {
        ...
        .then(() => HospitalEntity.deleteMany({ IsAdmin: false }).exec())
