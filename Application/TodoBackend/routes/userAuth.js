...
router.post('/admin/enroll', (req, res) => {
    ...
    EnrollAdmin(org) 
    .then(()=> res.status(200).json({message: "Enrollment was successfull"}))
    .catch((err) => {
        if(!(err instanceof ApiError))
        err = new ApiError(400, 'Bad Request', err.message).withDetails('Make sure the input is correct');
...
router.post('/client/new', (req, res) => {
    ...
    .then(()=> res.status(200).json({message: "Registration was successfull"}))
    .catch((err) => {
        if(!(err instanceof ApiError))
        err = new ApiError(400, 'Bad Request', err.message).withDetails('Make sure the input is correct');
        res.status(err.status).json(new response.errorResponse(err));
