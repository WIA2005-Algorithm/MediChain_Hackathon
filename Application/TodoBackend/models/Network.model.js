  ...
  new Schema(
    {
      ...
      Address: {
        ...
        validate: {
          validator: (v) =>
            /^[a-vx-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$/.test(
              v
            ),
          message: (props) =>
      ...
      Status: {
        ...
        default: {
          code: 0,
          message: "Not Started",
          description: "Fabric Network has not started yet"
        }
      }
    },
    { timestamps: true }
...
export const Organizations = mongoose.model(
  ...
  new Schema(
    ...
    Organizations.countDocuments({ OrganizationType: "Hospital" }).exec(function (
      err,
      count
    ) {
      if (err) return next(err);
      if (!p0ports[count]) return next(errors.maximum_organizations_reached);
