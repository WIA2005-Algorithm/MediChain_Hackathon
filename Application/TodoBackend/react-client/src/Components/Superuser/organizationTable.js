...
const EnhancedTableToolbar = (props) => {
  ...
  const deleteSelected = async () => {
    ...
    setPending({ code: 300, message: "Pending" });
    await stopNetwork(network.Name).then((res) => setPending(res.data.Status));
    var temprows = rows.map((x) => x);
    for (let k = 0; k < selected.length; k++) {
      const org = selected[k];
      for (let i = 0; i < temprows.length; i++)
        if (org === temprows[i].objID) {
          await deleteOrganization(network.Name, org, temprows[i].admin);
          temprows.splice(i, 1);
          break;
        }
    }
      ...
      <Dialog
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-slide-description">
            Please be aware than you cannot recover these deleted hospital organizations.
            Fabric network will automatically stop before deleting the hospital
        ...
        <DialogActions>
          {!completedDeletion ? (
            ...
            <>
              <Button onClick={toggleDialog} sx={{ fontWeight: "bold" }}>
                No
              </Button>
              <Button onClick={deleteSelected} sx={{ fontWeight: "bold" }}>
                Yes
              </Button>
            </>
          )}
        ...
        <>
          <Tooltip title="Enroll Selected Hospitals">
            <LoadingButton
              ...
              onClick={() => processRunningWarning(enrollSelected)}
              variant="contained"
              sx={{
                fontSize: "12px",
          ...
          <Tooltip title="Delete Selected">
            <IconButton onClick={() => processRunningWarning(toggleDialog)}>
        ...
        <>
          ...
          <Tooltip title="Start/Stop Network">
            <span style={{ minWidth: "fit-content" }}>
              {pending.code === 300 && <Status state={setPending} notis={notis} />}
              <LoadingButton
                variant="contained"
                color={
                  pending.code === 200
                    ? "success"
                    : pending.code === 500 || pending.code === 400
                    ? "error"
                    : "primary"
                }
...
export default function OrganizationTables({ nav, setNav, notis }) {
  ...
  return (
    <Routes>
      ...
      />
      <Route path="/new" element={<FullScreenDialog broadcastAlert={notis} />} />
      <Route
        path="/activity_logs"
        element={<ActivityLogs nav={nav} setNav={setNav} network={network} />}
      />
    </Routes>
  );
}
