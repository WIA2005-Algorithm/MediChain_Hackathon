...
import { getAllNetworks, getNetworkCount } from '../../APIs/Superuser/network.api';
...
function NothingYet(props) {
    return (
        <div style={{width: '100%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
            <CreateNetwork openForm={props.openForm} setOpenForm={props.setOpenForm} fetch={props.fetch}/>
            <Avatar alt='no networks added' src={nothing} sx={{width: '150px', height: '150px', mt: 1.8}}/>
...
function NetworkAvailable() {
    ...
    return(
        <SectionContainer 
        sx={{width:'fit-content', display: 'flex', justifyContent: 'stretch', alignItems: 'center', padding: '16px 24px'}}>
            <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                <Avatar alt='network' src={NetworkImage} sx={{width: '80px', height: '80px'}}/>
                <NetStatus 
                status={pending}
                setStatus={setpending}
                circlesx={{mt: 2, width: '25px', height: '25px'}} 
                sx={{color: 'primary.main', mt: 1, fontWeight: 'bold', textAlign: 'center'}}/>
            </div>
            <Box sx={{display: 'flex', flexDirection: 'column', alignItems: 'start', ml: 5, mr: 5}}>
                <Typography component='small' sx={{color: 'text.secondary', mt: 1, fontSize: '13px'}}>
                    Network Name
                </Typography>
                <Typography component='small' sx={{color: 'text.main', fontWeight: 'bold'}}>
                {response.Name}
                </Typography>
                <Typography component='small' sx={{color: 'text.secondary', mt: 1, fontSize: '13px'}}>
                    Network ID
...
function Load(props) {
    ...
    return <CircularProgress sx={{alignSelf: 'center'}}/>
        ...
        if(response.count === 0)
        return  <NothingYet openForm={props.openForm} setOpenForm={props.setOpenForm} fetch={fetchNetworkCount}/>
